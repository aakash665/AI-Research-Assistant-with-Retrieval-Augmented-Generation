...
from vector_db import close_vector_store_client
from ui.handler import (
    DEFAULT_APP_STATE,
    DEFAULT_APP_STATUS,
    handle_chat_message,
    handle_uploaded_documents,
...
with gr.Blocks(theme=gr.themes.Soft()) as demo:
    ...
    status = gr.Markdown(DEFAULT_APP_STATUS)
    ...
    demo.load(initialize_default_documents, inputs=app_state, outputs=[app_state, status])
    clear_button.click(reset_chat, inputs=app_state, outputs=[chatbot, app_state, status])
    uploads.change(
        handle_uploaded_documents,
    ...
    message.submit(
        handle_chat_message,
        inputs=[message, chatbot, app_state, uploads],
        outputs=[message, chatbot, app_state, status],
    )
